...
{
    // executa teste -----------------------------------------------------------
    bool testSucceeded = testHeapsort();
    printf(
        testSucceeded 
        ? "Teste da ordenação pelo Heapsort: Sucesso\n" 
        : "Teste da ordenação pelo Heapsort: Falhou\n");
    printf("\n");
    ...
    sprintf(outputFilename, "./results/meansHeapsort-%ld.dat", time(NULL));
